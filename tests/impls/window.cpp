...
Modifier::Modifier(int mods_)
    : shift(mods_ & GLFW_MOD_SHIFT), control(mods_ & GLFW_MOD_CONTROL),
      ...
      capsLock(mods_ & GLFW_MOD_CAPS_LOCK), numLock(mods_ & GLFW_MOD_NUM_LOCK) {
}
...
void Window::setEventCallbacks() {
  ...
  glfwSetFramebufferSizeCallback(mWindow, windowFramebufferSizeCallbackWrapper);
  glfwSetWindowContentScaleCallback(mWindow, windowContentScaleCallbackWrapper);
  glfwSetMouseButtonCallback(mWindow, mousebuttonCallbackWrapper);
  glfwSetCursorPosCallback(mWindow, cursorPositionCallbackWrapper);
  glfwSetCursorEnterCallback(mWindow, cursorEnterCallbackWrapper);
...
void Window::setCursor(Cursor const &cursor) { glfwSetCursor(mWindow, cursor); }
